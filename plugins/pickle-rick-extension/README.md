...
**This loop will continue until the task is complete or a `completion-promise` is fulfilled.** (Note: `max-iterations` and `max-time` are currently non-functional; individual workers have a 20m timeout).
...
_Tip: After drafting, you can use `/add-to-pickle-jar` to save the task for later execution._
...
_Note: This resumes the active session for your current working directory._
...
3. **Include Directories**: To ensure Pickle Rick can track its thoughts, manage Linear tickets, and persist session state, you **must** add the extension's data directory to your Gemini `includeDirectories` configuration (usually in your `.gemini/settings.json`).
...
**Pickle Rick is a fictional character.** This extension employs a creative, cynical, and arrogant persona inspired by the show _Rick and Morty_ for the purpose of demonstrating advanced agentic workflows and "God Mode" engineering principles.

...
import {
  ...
  listSessions,
  GLOBAL_SESSIONS_DIR,
} from "./services/config/state.js";
import { createProvider } from "./services/providers/index.js";
import {
  validateSettings,
  loadSettingsWithValidation,
  saveSettings,
} from "./services/config/settings.js";
...
program
  ...
  .option(
    ...
    "Stop when this text is found in output",
    "I AM DONE"
  )
  ...
  .action(async (prompt, options) => {
    ...
    if (options.resume) {
      ...
      console.log(
        pc.green(`🥒 Pickle Rick is resuming session at ${options.resume}...`)
      );
    } else {
...
program
  ...
  .action(async () => {
    ...
    for (const s of sessions) {
      ...
      console.log(
        `${pc.dim(date)} | ${pc.bold(s.status)} | ${pc.white(s.original_prompt.substring(0, 50))}...`
      );
...
program
  ...
  .action(async options => {
    ...
    if (settings.model?.model) {
      ...
      console.log(
        `  Model: ${pc.dim("Not configured (will use provider default)")}`
      );
    }
    console.log("");

...
describe("PickleTaskSource Sequencing", () => {
  ...
  test.skip("should sort tickets by order field", async () => {
    const testDir = join(
      process.cwd(),
      `.tmp_test_session_${Date.now()}_${Math.random().toString(36).slice(2)}`
    );
    ...
    await writeFile(
      join(testDir, "ticket_3.md"),
      `---\nid: t3\ntitle: Third\nstatus: Triage\norder: 30\n---\nBody`
    );
    await writeFile(
      join(testDir, "ticket_1.md"),
      `---\nid: t1\ntitle: First\nstatus: Triage\norder: 10\n---\nBody`
    );
    await writeFile(
      join(testDir, "ticket_2.md"),
      `---\nid: t2\ntitle: Second\nstatus: Triage\norder: 20\n---\nBody`
    );
  ...
  test.skip("should fallback to birthtime if order is identical", async () => {
    ...
    await writeFile(
      join(testDir, "ticket_a.md"),
      `---\nid: ta\ntitle: A\nstatus: Triage\norder: 10\n---\nBody`
    );
    // Wait a bit to ensure different birthtime
    await new Promise(r => setTimeout(r, 100));
    await writeFile(
      join(testDir, "ticket_b.md"),
      `---\nid: tb\ntitle: B\nstatus: Triage\norder: 10\n---\nBody`
    );

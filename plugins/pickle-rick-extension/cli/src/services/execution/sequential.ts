...
export class SequentialExecutor {
  ...
  async run(): Promise<ExecutionResult> {
    ...
    const baseBranch =
      (await getCurrentBranch(this.state.working_dir)) || "main";
    let sessionWorktree: { worktreeDir: string; branchName: string } | null =
      null;
...
🥒 Iteration ${this.state.iteration}`)
        ...
        // Check limits
        if (
          this.state.max_iterations > 0 &&
          this.state.iteration > this.state.max_iterations
        ) {
        ...
        if (task.id.startsWith("phase-") === false) {
          if (!sessionWorktree) {
            try {
              ...
              await log(
                pc.dim(`🏗️  Session Worktree: ${sessionWorktree.worktreeDir}`)
              );
        ...
        const iterationLogFile = join(
          debugDir,
          `iteration_${this.state.iteration}_log.txt`
        );
        try {
        ...
        const logIteration = async (msg: string) => {
          try {
            await appendFile(iterationLogFile, msg);
          } catch (e) {}
        };
        ...
        const result = await this.provider.executeStreaming!(
          ...
          async (step, content) => {
            if (content) {
              ...
              try {
                await appendFile(logFile, stepMsg);
              } catch (e) {}
...
❌ Engine Error: ${singleLineError}\n`
        ...
        const promiseFulfilled =
          ...
          result.response.includes(
            `<promise>${this.state.completion_promise}</promise>`
          );
                ...
                if (ghAvailable) {
                  ...
                  try {
                    ...
                    if (prUrl) {
                      ...
                      await log(
                        pc.red(
                          "⚠️ Failed to create PR. Saving description to file..."
                        )
                      );
                      const prDescPath = join(
                        this.state.session_dir,
                        "pr_description.md"
                      );
                      await writeFile(
                      ...
                      await log(
                        pc.yellow(`📄 PR description saved to: ${prDescPath}`)
                      );
                    }
                  } catch (e) {
                  ...
                  await log(
                    pc.yellow(
                      "⚠️ GitHub CLI (gh) not installed or not authenticated."
                    )
                  );
                  ...
                  await log(
                    pc.dim(`To create PR manually, push the branch and use:`)
                  );
                  await log(
                    pc.dim(`  git push -u origin ${sessionWorktree.branchName}`)
                  );
                  await log(
                    pc.dim(
                      `  gh pr create --base ${baseBranch} --head ${sessionWorktree.branchName}`
                    )
                  );
                }

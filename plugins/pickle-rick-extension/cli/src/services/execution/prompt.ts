...
export async function buildPrompt(
  state: SessionState,
  task?: Task,
  overrides?: { sessionDir?: string; workingDir?: string }
): Promise<string> {
  ...
  if (task) {
    ...
    Output: "Tickets Created. I AM DONE"`;
      ...
      if (!existsSync(ticketPath)) {
        throw new Error(
          `CRITICAL ERROR: Ticket file missing at ${ticketPath}. The session state is corrupted or the file was deleted. Execution halted.`
        );
      }
      ...
      try {
        ...
        const statusMatch = ticketContent.match(
          /status:\s*["']?([^"'\n]+)["']?/i
        );
      ...
      const ticketIsDone =
        ticketStatus === "done" || ticketStatus === "in progress";
      ...
      const researchApproved =
        ticketStatus.includes("ready for plan") ||
        ticketStatus.includes("plan") ||
        planApproved;
      ...
      const researchPath =
        researchDoc || researchReviewDoc || join(ticketDir, "research.md");

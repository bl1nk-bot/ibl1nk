...
describe("SequentialExecutor", () => {
  ...
  beforeEach(() => {
    ...
    spies = [
      ...
      spyOn(PickleTaskSource.prototype, "getNextTask").mockImplementation(
        async function (this: any) {
          if (!this._mockTasks) {
            this._mockTasks = [
              { id: "task1", title: "Task 1", body: "", completed: false },
            ];
          }
          return this._mockTasks.shift() || null;
        }
      ),
      spyOn(PickleTaskSource.prototype, "markComplete").mockResolvedValue(
        undefined
      ),

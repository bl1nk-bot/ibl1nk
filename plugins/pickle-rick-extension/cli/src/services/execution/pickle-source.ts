...
export class PickleTaskSource implements TaskSource {
  ...
  async getTask(id: string): Promise<Task | null> {
    if (id === "phase-prd" || id === "phase-breakdown") {
      ...
      if (id === "phase-breakdown")
        return {
          id,
          title: "Breakdown",
          completed: state.step !== "breakdown" && state.step !== "prd",
          body: "Breakdown...",
        };
      return null;
    }
    ...
    const isDone =
      status.toLowerCase() === "done" || status.toLowerCase() === "canceled";
  ...
  async countRemaining(): Promise<number> {
    const state = await this.getState();
    if (state.step === "prd")
      return 2 + (await this.countTickets(state.session_dir, false)); // PRD + Breakdown + Tickets
    if (state.step === "breakdown")
      return 1 + (await this.countTickets(state.session_dir, false));
    return this.countTickets(state.session_dir, false);
  }

  // --- Helpers ---

  private async findTicketFile(
    dir: string,
    id: string
  ): Promise<string | null> {
    // Search recursively
  ...
  private async findNextTicket(dir: string): Promise<Task | null> {
    ...
    const implementable = allTickets.filter(t => {
      const isParentId =
        t.id === "parent" ||
        t.id === "linear_ticket_parent" ||
        t.id === "task_priority_parent";
      ...
      const isRootFile =
        t.metadata?.path &&
        !join(this.sessionDir, t.id).includes(t.metadata.path);
          ...
          if (entryStat.isDirectory()) {
            ...
            if (idMatch) {
              ...
              if (filenameMatch) {
                ...
                console.warn(
                  `⚠️ Warning: No frontmatter ID in ${entry}. Using filename ID: ${id}`
                );
              }
            }

...
export async function runWorker(
  ...
  if (ticketPath.endsWith(".md")) {
    ...
    existsSync(ticketPath) &&
    existsSync(join(ticketPath, "..", "state.json"))
  ) {
    ...
    try {
      ...
      if (maxMins > 0 && startTime > 0) {
        ...
        if (remaining * 1000 < timeoutMs) {
          ...
          console.log(
            pc.yellow(
              `⚠️  Worker timeout clamped: ${clamped}s (Global Session Limit)`
            )
          );
        }
      }
  ...
  if (skillName) {
    ...
    if (skillPath && existsSync(skillPath)) {
      ...
      console.log(
        pc.dim(`📄 Skill content loaded (${skillContent.length} chars)`)
      );
      ...
      console.warn(
        pc.yellow(`⚠️ Skill '${skillName}' not found at ${skillPath}`)
      );
    }
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", code => {
      ...
      const isSuccess =
        outputBuffer.includes("<promise>I AM DONE</promise>") ||
        outputBuffer.includes("I AM DONE");

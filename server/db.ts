import { eq, or, gte, and } from "drizzle-orm"; // OPTIMIZATION: Static imports avoid module resolution overhead
...
export async function getCharacterRelationships(characterId: number) {
  const db = await getDb();
  if (!db) return [];
  // OPTIMIZATION: Removed dynamic import to improve performance
  return db
    .select()

...
export const outlinesRouter = router({
  ...
  storyOverview: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const outline = await getOutlineByIdForUser(input.outlineId, ctx.user.id);
      if (!outline) throw new Error("Outline not found");
      // ⚡ Bolt: Parallelize independent read queries for chapters and characters after outline authorization
      // This reduces sequential database roundtrips and improves story overview load time.

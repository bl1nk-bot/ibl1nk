...
export const outlinesRouter = router({
  ...
  storyOverview: protectedProcedure
    .input(z.object({ outlineId: z.number() }))
    .query(async ({ input }) => {
      // ⚡ Bolt Optimization: Use Promise.all to fetch outline, chapters, and characters concurrently
      // Expected impact: Reduces latency from O(N) sequential queries to O(Max(query_time)), roughly cutting DB wait time by up to 2/3.

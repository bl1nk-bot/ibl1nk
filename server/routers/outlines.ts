...
export const outlinesRouter = router({
  ...
  storyOverview: protectedProcedure
    .input(z.object({ outlineId: z.number() }))
    .query(async ({ input }) => {
      // ⚡ Bolt Performance Optimization: Parallelized database queries
      // Why: Fetches for outline, chapters, and characters are independent read operations.
      // Impact: Reduces overall response latency from the sum of sequential query times to the maximum query time.

...
export const outlinesRouter = router({
  ...
  storyOverview: protectedProcedure
    .input(z.object({ outlineId: z.number() }))
    .query(async ({ input }) => {
      // Optimization: Parallelize independent database queries to reduce overall latency.
      // This reduces response time from (outline time + chapters time + characters time) to the maximum of the three query times.

...
## 2024-06-26 - Parallelize Independent Database Queries
**Learning:** Sequential database queries within the same request handler (e.g., fetching outline, chapters, and characters) significantly increase response latency as the total time becomes the sum of all individual query times.
**Action:** Always use `Promise.all` to fetch independent read-only database queries concurrently.

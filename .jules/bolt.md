...
## 2026-07-01 - Parallelize independent DB queries
**Learning:** Sequential await calls for independent database queries (e.g. `getOutlineById`, `getChaptersByOutlineId`, `getCharactersByOutlineId`) create unnecessary latency in route handlers like `storyOverview`.
**Action:** Always group independent read operations inside `Promise.all()` to reduce execution time from the sum of sequential query times to the maximum individual query time.

...
## 2026-05-15 - Safely Parallelizing tRPC Queries
**Learning:** Parallelizing database queries with `Promise.all` is highly effective for performance, but in tRPC route handlers, all queries running concurrently can bypass authorization checks and create unnecessary database load if the initial auth-gate fails.
**Action:** Always `await` authorization-gated queries (e.g., ownership checks like `getOutlineByIdForUser`) first, and only parallelize the subsequent independent read queries.

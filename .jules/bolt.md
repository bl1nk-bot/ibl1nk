...
## 2024-08-01 - Parallelize Independent DB Queries in tRPC Handlers
**Learning:** Sequential database queries (like `getOutlineById`, `getChaptersByOutlineId`, `getCharactersByOutlineId`) in tRPC route handlers like `storyOverview` cause latency to compound (T1 + T2 + T3).
**Action:** Use `Promise.all` to parallelize independent queries where possible, reducing the overall execution time to the maximum of the individual queries (Max(T1, T2, T3)).

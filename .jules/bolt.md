...
## 2024-06-07 - Parallelize independent queries in tRPC routers
**Learning:** Sequential await calls for independent database reads (like fetching outline, chapters, and characters) in tRPC route handlers increase overall response latency to the sum of all query times.
**Action:** Use Promise.all to parallelize independent read operations in route handlers, reducing the response latency to the maximum query time.

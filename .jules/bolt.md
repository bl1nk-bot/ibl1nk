...
## 2024-07-26 - Parallelize independent queries with Promise.all
**Learning:** Sequential await calls for independent database reads (e.g., fetching outline, chapters, and characters) unnecessarily increase endpoint response latency to the sum of individual query times.
**Action:** Always use `Promise.all` to fetch independent read operations concurrently in tRPC endpoints to reduce latency to the maximum individual query time.

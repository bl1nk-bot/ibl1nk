...
## 2026-08-14 - Parallelizing Independent DB Queries after Auth

**Learning:** Running independent database queries (like fetching chapters and characters) sequentially after an authorization-gated query unnecessarily increases route latency.
**Action:** Once an authorization-gated query (like `getOutlineByIdForUser`) resolves, use `Promise.all` to fetch the remaining independent relational data concurrently.

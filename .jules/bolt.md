...
**Learning:** Dynamic imports of `drizzle-orm` operators (e.g., `const { or } = await import("drizzle-orm")`) inside frequently called database query functions cause performance bottlenecks and module resolution overhead on every query.
**Action:** Use top-level static imports for `drizzle-orm` operators (e.g., `import { and, eq, or, gte } from "drizzle-orm"`) to prevent unnecessary module resolution overhead and improve database query performance.

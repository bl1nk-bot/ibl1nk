...
## 2024-06-08 - Use Promise.all for independent queries in tRPC routers
**Learning:** Sequential await calls for independent database queries in tRPC routers (e.g., fetching outline, chapters, and characters) unnecessarily increase total response latency. The total time is the sum of all queries.
**Action:** Always use `Promise.all` to fetch independent queries concurrently in tRPC routers to reduce latency. The overall response time becomes the time of the longest query.

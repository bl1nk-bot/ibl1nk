...
## 2024-07-04 - Parallelize independent queries in tRPC routers
**Learning:** Sequential execution of independent database queries (e.g. fetching outline, chapters, and characters consecutively) in tRPC route handlers creates unnecessary latency overhead.
**Action:** Always prefer `Promise.all` to execute independent database queries concurrently, which minimizes the overall response latency to the maximum duration among the queries.

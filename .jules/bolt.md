...
## 2024-05-24 - Parallelizing Independent DB Queries

**Learning:** When executing multiple independent database queries (e.g., fetching chapters and characters), running them sequentially creates unnecessary network and DB latency. However, it is critical to always `await` authorization-gated queries (e.g., ownership checks) first before parallelizing the rest to prevent security bypasses.
**Action:** Use `Promise.all` to parallelize independent read operations in tRPC routes, but only after executing and awaiting any necessary authorization/ownership checks.

...
## 2024-06-08 - Use `Promise.all` for independent DB queries in tRPC routers
**Learning:** In tRPC routes handling compound responses (e.g. `storyOverview`), using sequential `await` calls for independent database queries aggregates the total latency of all requests into the response time.
**Action:** Always wrap independent database queries within `Promise.all` to execute them concurrently, reducing total response latency from the sum of all execution times to the maximum individual query time.

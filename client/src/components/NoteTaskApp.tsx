...
// Cached div for high-performance HTML-to-text extraction
const htmlToTextDiv =
...
const _NoteTaskAppWithRouter = () => {
      ...
      fuseNotesRef.current = new window.Fuse(notes, {
        keys: [
          { name: "title", weight: 0.4 },
          {
            name: "content",
            weight: 0.3,
            getFn: (note: AppNote) => getPlainText(note.content),
          },
      ...
      .filter(
        note =>
          ...
          (plainTextCharCount(note.content) > 0
            ...
            : false) ||
          note.tags.some(tag =>
            tag.toLowerCase().includes(noteSearchTerm.toLowerCase())
          )
      )
      .sort(
        (a, b) =>
  ...
  const filteredLoreEntries = useMemo(() => {
    ...
    return baseLore
      .filter(
        entry =>
          entry.title.toLowerCase().includes(loreSearchTerm.toLowerCase()) ||
          entry.content.toLowerCase().includes(loreSearchTerm.toLowerCase()) ||
          entry.tags.some(tag =>
            tag.toLowerCase().includes(loreSearchTerm.toLowerCase())
          ) ||
          entry.type.toLowerCase().includes(loreSearchTerm.toLowerCase())
      )
  ...
  const handleSubmitAi = async (selectedContextLoreIds?: string[]) => {
    ...
    if (
      ...
      const contextNotes = projectNotesToConsider
        ...
        .map(
          n =>
            `โน้ต "${n.title}": ${plainTextCharCount(n.content) > 0 ? getPlainText(n.content).substring(0, PROJECT_CONTEXT_MAX_NOTE_CHARS) : ""}...`
        )
        .join("\n");
  ...
  const handleCopyAiResponse = async () => {
    if (aiResponseRef.current) {
      let textToCopy = getPlainText(aiResponseRef.current.innerHTML);
      if (
        textToCopy &&
  ...
  const handleSaveAiResponseAsNote = () => {
    if (
      ...
      if (match) {
        ...
        const plainTextResponse = getPlainText(
